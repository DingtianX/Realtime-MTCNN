...
if __name__ == "__main__":
    vs = WebcamVideoStream(src=1, width=640, height=480, fps=30).start()
    vs1 = WebcamVideoStream(src=2, width=640, height=480, fps=30).start()
    while True:
        frame0 = vs.read()
        frame1 = vs1.read()
        #frame0 = frame0[60:420, :]
        #frame1 =frame1[60:420, :]
        frame = np.hstack((frame0, frame1))
        frame_4 = np.vstack((frame, frame))
        timestart = time.clock()
        faces, points = getFace(frame_4, compress_frame=True)
        print(time.clock()-timestart)
        for face in faces:
            cv2.rectangle(frame_4, (face['rect'][0], face['rect'][1]),
                          (face['rect'][2], face['rect'][3]), (0, 0, 225), 2)
            # landmarks 绘制人脸关键点
        # for p in points.T:
        #     for i in range(5):
        #         cv2.circle(frame, (p[i], p[i + 5]), 1, (225, 0, 0), 2)
        cv2.imshow("faces", frame_4)
